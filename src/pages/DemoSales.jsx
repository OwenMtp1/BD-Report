...
// Ouvre la démo sur sa propre page. Elle n'est plus un calque : deux applications
// montées côte à côte écoutent les mêmes événements de navigation et se disputent la
// main, ce qui faisait dérailler la visite guidée de la formation.
export function openDemoPage() {
  const url = window.location.pathname + window.location.search + '#/demo'
  const win = window.open(url, '_blank')
  if (!win) { window.location.hash = '/demo'; window.location.reload() }
}
  ...
  return (
    <div>
      <div className="rounded-2xl p-6 sm:p-8 text-white relative overflow-hidden" style={{ background: 'linear-gradient(135deg,#10162e,#1e2a52 45%,#14346b)' }}>
        ...
        <button className="btn-primary mt-5" onClick={openDemoPage}><Play size={16} /> Démarrer la démo</button>
