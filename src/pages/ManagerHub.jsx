          ...
          <div className="flex gap-1.5 overflow-x-auto border-b border-line">
            {tabs.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`flex items-center gap-1.5 px-3 py-2 text-sm font-semibold rounded-t-lg whitespace-nowrap border-b-2 -mb-px ${current?.id === t.id ? 'border-brand text-brand' : 'border-transparent text-muted hover:bg-surface'}`}>
